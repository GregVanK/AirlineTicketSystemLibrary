...
public class FlightRepository {
    ...
    private final String SPROC_DELETE_FLIGHT = "CALL DeleteFlight(?)";
    private final String SPROC_SEARCH_FLIGHTS = "CALL SearchFlights(?,?,?,?,?)";
	...
	try {
	    ...
	    DALRdbms.executeNonQuery(SPROC_DELETE_FLIGHT, params.toArray());
	} catch (Exception e) {
	    System.out.println(e.getMessage());
	}
    }
	...
	try {
	    ArrayList<Object> params = new ArrayList() {
		{
		    add(flight.getDepartureAirport());
		    add(flight.getArrivalAirport());
		    ...
		    if(flight.getArrivalDate().equals(""))
			add(null);
		    else
			add(flight.getArrivalDate());
		    add(flight.getAvailableSeats());
	    ...
	    CachedRowSet rs = DALRdbms.executeFill(SPROC_SEARCH_FLIGHTS, params.toArray());
	    flights = toListOfFlights(rs);
	} catch (Exception e) {
	    System.out.println(e.getMessage());
	}
	return flights;
